  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq >= radsum * radsum) {
        ...
        damp =  gamman * vnnr * rsqinv; //HC: removed meff from damping force, this makes the units of gamman = mass/time and not 1./time
        ...
        fs1 = -(kt * shear[0] +  gammat * vtr1); //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
        fs2 = -(kt * shear[1] +  gammat * vtr2); //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
        fs3 = -(kt * shear[2] +  gammat * vtr3); //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
        ...
        if (fs > fn) {
          if (shrmag != 0.0) {
            shear[0] =
                (fn / fs) * (shear[0] + gammat * vtr1 / kt) - gammat * vtr1 / kt; //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
            shear[1] =
                (fn / fs) * (shear[1] + gammat * vtr2 / kt) - gammat * vtr2 / kt; //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
            shear[2] =
                (fn / fs) * (shear[2] + gammat * vtr3 / kt) - gammat * vtr3 / kt; //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
            fs1 *= fn / fs;
            fs2 *= fn / fs;
            fs3 *= fn / fs;
          } else
