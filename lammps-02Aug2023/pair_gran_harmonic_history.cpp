...
#include "pair_gran_harmonic_history.h"
...
PairGranHarmonicHistory::PairGranHarmonicHistory(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 1;
...
PairGranHarmonicHistory::~PairGranHarmonicHistory()
{
  if (copymode) return;
...
void PairGranHarmonicHistory::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq >= radsum * radsum) {
        ...
        damp = meff * gamman * vnnr * rsqinv; //HC: removed meff from damping force, this makes the units of gamman = mass/time and not 1./time
        ccel = kn * ((radsum - r) / radsum) * rinv - damp; //HC: normalized the normal, harmonic, force by (radsum)   
        ...
        fs1 = -(kt * shear[0] + meff * gammat * vtr1); //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
        fs2 = -(kt * shear[1] + meff * gammat * vtr2); //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
        fs3 = -(kt * shear[2] + meff * gammat * vtr3); //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
        ...
        if (fs > fn) {
          if (shrmag != 0.0) {
            shear[0] =
                (fn / fs) * (shear[0] + meff * gammat * vtr1 / kt) - meff * gammat * vtr1 / kt; //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
            shear[1] =
                (fn / fs) * (shear[1] + meff * gammat * vtr2 / kt) - meff * gammat * vtr2 / kt; //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
            shear[2] =
                (fn / fs) * (shear[2] + meff * gammat * vtr3 / kt) - meff * gammat * vtr3 / kt; //HC: removed meff from damping force, this makes the units of gammat = mass/time and not 1./time
            fs1 *= fn / fs;
            fs2 *= fn / fs;
            fs3 *= fn / fs;
          } else
...
void PairGranHarmonicHistory::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairGranHarmonicHistory::settings(int narg, char **arg)
{
...
void PairGranHarmonicHistory::coeff(int narg, char **arg)
{
...
void PairGranHarmonicHistory::init_style()
{
  int i;
...
double PairGranHarmonicHistory::init_one(int i, int j)
{
  if (!allocated) allocate();
...
void PairGranHarmonicHistory::write_restart(FILE *fp)
{
...
void PairGranHarmonicHistory::read_restart(FILE *fp)
{
...
void PairGranHarmonicHistory::write_restart_settings(FILE *fp)
{
...
void PairGranHarmonicHistory::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairGranHarmonicHistory::reset_dt()
{
  dt = update->dt;
}
...
double PairGranHarmonicHistory::single(int i, int j, int /*itype*/, int /*jtype*/, double rsq,
...
int PairGranHarmonicHistory::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/,
                                            int * /*pbc*/)
{
  int i, j, m;
...
void PairGranHarmonicHistory::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
...
double PairGranHarmonicHistory::memory_usage()
{
...
double PairGranHarmonicHistory::atom2cut(int i)
{
...
double PairGranHarmonicHistory::radii2cut(double r1, double r2)
{
  double cut = r1 + r2;
  return cut;
}
